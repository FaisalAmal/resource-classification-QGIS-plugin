...
**Example:**
- Drillhole A: nearest neighbor = 65m → **Measured**
- Drillhole B: nearest neighbor = 95m → **Indicated**
- Drillhole C: nearest neighbor = 150m → **Inferred**
...
Result: 2 clusters
- Cluster 1: A,B,C (3 holes) → REJECTED (< 4 holes)
- Cluster 2: D,E,F,G (4 holes) → VALID ✓
```
...
```python
max_area = distance² × area_factor
# For 71m: max_area = 71² × 0.6 = 3,020 m²
```

**Geological Reasoning:**
- If drillholes are 71m apart, each drillhole represents roughly a 71m × 71m area
...
```python
# Rule: At least 2 edges ≤ max_edge
# Longest edge can be up to 1.5 × max_edge
```

**Why:** Real drilling rarely follows perfect grids. This allows:
- Grid sides: 71m ✓
- Grid diagonals: ~100m (√2 × 71m ≈ 100m) ✓
- Skip connections: 142m ✗
...
- A,B,C: Connected by valid triangles → Sub-cluster 1 ✓
- X,Y,Z: Connected by valid triangles → Sub-cluster 2 ✓
- Gap too large between C and X → Separate polygons!
```
...
- Standard practice: buffer = ~35% of spacing
  - 71m spacing → 25m buffer (35%)
  - 121m spacing → 50m buffer (41%)
...
## **7. Hierarchy: Measured ⊂ Indicated ⊂ Inferred**
...
```
        ┌─────────────────────────┐
        │      Inferred           │  (Widest spacing)
        │   ┌────────────────┐    │
        │   │   Indicated    │    │  (Medium spacing)
        │   │  ┌─────────┐   │    │
        │   │  │Measured │   │    │  (Tightest spacing)
        │   │  └─────────┘   │    │
        │   └────────────────┘    │
        └─────────────────────────┘
```
...
| `edge_tolerance=1.5` | 1.5× | Allows diagonals up to 1.5× grid spacing |
...
This approach ensures geologically realistic, statistically valid resource classifications! 🎯
